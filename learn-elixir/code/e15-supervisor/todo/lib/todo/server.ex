defmodule Todo.Server do
  ...
  @spec handle_cast({:add, Item.t()}, Todo.List.t()) :: {:noreply, Todo.List.t()}
  def handle_cast({:add, item}, todos) when is_map(item) and is_list(todos) do
    {:noreply, [item | todos]}
  end

  @spec handle_cast({:remove, Item.t()}, Todo.List.t()) :: {:noreply, Todo.List.t()}
  def handle_cast({:remove, item}, todos) when is_map(item) and is_list(todos) do
    {:noreply, List.delete(todos, item)}
  end

  @spec handle_cast({:complete, Item.t() | String.t()}, Todo.List.t()) :: {:noreply, Todo.List.t()}
  ...
  @spec handle_cast(:pop_front, Todo.List.t()) :: {:noreply, Todo.List.t()}
  def handle_cast(:pop_front, todos) when is_list(todos) do
    {_, rest} = List.pop_at(todos, 0)
    {:noreply, rest}
  end

  @spec handle_call(:get_items, any(), Todo.List.t()) :: {:reply, Todo.List.t(), Todo.List.t()}
